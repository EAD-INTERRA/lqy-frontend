<template>
  <div class="p-6 space-y-6">
    <!-- Header Section -->
    <div class="space-y-2">
      <h1 class="text-2xl font-bold text-black-900">Stakeholders</h1>
      <p class="text-black-600">This is the list of all stakeholders on the system</p>
    </div>
      ...
      <div class="bg-white shadow-lg p-5 lg:p-5 rounded-lg lg:rounded-2xl mb-2.5">
        ...
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center space-x-2">
            ...
            <span class="text-black-700 font-medium">Stake Holders List</span>
          </div>
          ...
          <div class="flex items-center space-x-3">
            ...
            <!-- Search Input -->
            <div class="relative">
              <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-black-400" fill="none"
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="(item, index) in paginatedRows" :key="index" class="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td class="py-4 px-4">
                  <button v-if="getStatusText(item.user?.status) === 'Pending'" @click="openModal(item)"
        ...
        <div class="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
          ...
          <div class="flex items-center space-x-2">
            ...
            <button @click="setCurrentPage(totalPages)" v-if="totalPages > 5 && currentPage < totalPages - 2"
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
          ...
          <div
            class="bg-white rounded-lg shadow-lg w-fit max-w-2xl p-6 space-y-6 relative max-h-[90vh] overflow-y-auto">
            <!-- Close Button -->
            <button @click="showModal = false"
              class="absolute right-4 top-4 text-black-400 hover:text-black-600 text-xl">
              ✕
            </button>
            ...
            <div class="space-y-4 pr-8">
              ...
              <div class="border rounded-md w-fit flex items-center justify-between bg-gray-100 px-3 py-2">
                <div class="flex items-center space-x-2">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 text-black-600">
                  ...
                  <span class="text-black-700">Download file</span>
                </div>
                <button class="hover:bg-gray-200 p-1 rounded transition ml-4" @click="downloadFile">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                    stroke="currentColor" class="w-5 h-5 text-black-600">
                    <path stroke-linecap="round" stroke-linejoin="round"
              ...
              <div class="flex justify-start gap-4 pt-4">
                <button @click="approve(selectedStakeholder?.user?.id, true)"
                  class="flex-1 px-4 py-2 bg-[#034579] text-white rounded hover:bg-blue-900 disabled:opacity-50 disabled:cursor-not-allowed"
...
const filteredRequests = computed(() => {
  ...
  if (statusFilter.value) {
    filtered = filtered.filter(request => {
      let status = request.user?.status;
...
onMounted(async () => {
  try {
    ...
    console.log("Fetched stakeholders:", response);
