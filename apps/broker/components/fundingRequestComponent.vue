<template>
  <div class="mx-auto p-4">
    <button @click="$emit('go-back')" class="text-black text-[18px] hover:underline mb-4 inline-block flex h-fit"><svg
      ...
      <div class="bg-white shadow-custom-heavy p-5 rounded-[16px]">
        ...
        <form @submit.prevent="submitForm" class="grid grid-cols-1 md:grid-cols-2 gap-6 font-ub">
          ...
          <div class="space-y-4">
            <!-- CHN -->
            <div>
              <label for="chn" class="block text-[14px] font-ub text-black">CHN Number</label>
              <input id="chn" v-model="form.chn" type="text" maxlength="12" placeholder="Enter CHN number"
                class="form-input" />
            ...
            <div>
              ...
              <input id="amount" v-model="form.amount" type="number" placeholder="Enter amount" class="form-input" />
              <p v-if="errors.amount" class="text-red-500 text-sm mt-1">
                {{ errors.amount }}
              </p>
            </div>
            <div>
              <label for="bank_id" class="block text-[14px] font-ub text-black">Bank</label>
              <select id="bank_id" v-model="bank_id" class="form-input">
          ...
          <div class="space-y-4">
            <div>
              <label for="purpose" class="block text-[14px] font-ub text-black">Purpose of Loan</label>
              <textarea id="purpose" v-model="form.purpose" rows="9" placeholder="Describe the purpose of the loan"
