<template>
  <div>
    <div v-if="showApprovalSuccessModal">
      <ApprovalSuccessModal @close="showApprovalSuccessModal = false; emit('close')" />
    </div>
    <div v-else class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl mx-4 relative">
        ...
        <div class="p-6 space-y-6">
          ...
          <!-- <div class="flex gap-4 pt-4">
            ...
            <div v-else-if="showRejectSuccessModal">
              <RejectSuccessModal @close="showRejectSuccessModal = false; emit('close')" />
            </div>
            <div v-else class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
              <button @click="handleAccept(props.requestId)"
...
import { ref } from 'vue';
import ApprovalSuccessModal from './ApprovalSuccessModal.vue';
import RejectSuccessModal from './RejectSuccessModal.vue';
...
const handleAccept = async (id: string) => {
  ...
  try {
    ...
    toast.error(error?.response?.data?.body || 'Error accepting request');
  } finally {
