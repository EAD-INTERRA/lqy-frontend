<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 space-y-6 relative">
      ...
      <h2 class="text-xl font-semibold text-red-600">Margin Request</h2>
      ...
      <div class="space-y-2">
        <div>
          ...
          <!-- <p class="text-base font-medium">{{ requestDetails?.user?.profile?.first_name }} {{ requestDetails?.user?.profile?.last_name }}</p> -->
        </div>
        <div>
          <span class="text-sm text-gray-500">CHN Number:</span>
          <!-- <p class="text-base font-medium">{{ requestDetails?.chn }}</p> -->
        </div>
        <div>
          <span class="text-sm text-gray-500">Email:</span>
          <!-- <p class="text-base font-medium">{{ requestDetails?.user?.email }}</p> -->
        </div>

        <div class="flex justify-end gap-4 pt-4">
          <!-- <button
            @click="handleReject(requestId)"
        ...
        >
          ...
          @click="handleAccept(requestId)"
            ...
            :disabled="loading"
          :class="loading ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'"
        >
          <template v-if="loading">
            <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
